...
export class BranchFromComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    this.cSub = this.cityServ.GetOptions(this.baseURL+"cityOptions").subscribe({
      next: (value) => {
        console.log(value);
        this.cities = value;
    ...
    this.zSub = this.route.params.subscribe({
      next: (params) => {
        ...
        if (this.branchId) {
          this.bSub = this.branchServ.GetById(this.baseURL+"branches/"+this.branchId).subscribe({
            next: (value) => {
              this.branch = value;
  ...
  onSubmit() {
    ...
    if (this.branchId) {
      this.brSub = this.branchServ.Edit(this.baseURL+"branches/"+this.branchId, this.branch).subscribe({
        next: (value) => {
      ...
      this.brSub = this.branchServ.Add(this.baseURL+"branches",newBranch).subscribe({
        next: (value) => {
          this.routing.navigate(['/admin/branch'])
        },
        error: (error) => {
