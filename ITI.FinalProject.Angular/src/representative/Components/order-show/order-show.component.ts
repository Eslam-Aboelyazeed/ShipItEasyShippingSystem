...
export class OrderShowComponent implements OnInit, OnDestroy {
  ...
  getOrders(){
    ...
    {
      getPageURL=`${this.baseUrl}RepresentativeOrderPage/${this.authService.getRepresentativeId()}?pageNumber=${this.currentPage}&pageSize=${this.selectedEntries}`
    }
    else{
      getPageURL=`${this.baseUrl}RepresentativeOrderPage/${this.authService.getRepresentativeId()}?pageNumber=${this.currentPage}&pageSize=${this.selectedEntries}&orderStatus=${this.orderStatus}`
    }
